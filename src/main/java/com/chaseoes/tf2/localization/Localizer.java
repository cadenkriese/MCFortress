...
public class Localizer {
	...
	public static void sendGameEndStats(Player p, Team t, int kills, int deaths, int cps, int hks, String win, boolean pWin)
	{
		...
		metal = metal+gp.getGameCredits();
		metal = metal*BoosterStatusFile.getFile().getBoost();
		p.sendMessage("");
		p.sendMessage("");
		p.sendMessage("");
		p.sendMessage("");
		...
		p.sendMessage("");
		p.sendMessage("");
		p.sendMessage("§e§m-----------------§e Game Stats §e§m-------------------");
		p.sendMessage("");
		p.sendMessage("     §8Winning Team: "+win.toUpperCase());
		p.sendMessage("");
		p.sendMessage("     §8Kills: §f"+kills);
		p.sendMessage("");
		p.sendMessage("     §8Highest KillStreak: §f"+hks);
		p.sendMessage("");
		p.sendMessage("     §8Deaths: §f"+deaths);
		if (t == Team.RED)
		{
			p.sendMessage("");
			p.sendMessage("     §8Points Captured: §f"+cps);
			p.sendMessage("");
			p.sendMessage("     §8Credits Earned: §f"+metal+BoosterStatusFile.getFile().getBoosterMessage());
			p.sendMessage("§e§m-----------------§e Game Stats §e§m-------------------");
		}
		else
		{
			p.sendMessage("");
			p.sendMessage("     §8Metal Earned: §f"+metal+BoosterStatusFile.getFile().getBoosterMessage());
			p.sendMessage("§e§m-----------------§e Game Stats §e§m-------------------");
		}
		TF2.getInstance().addCredits(p, metal);
	}
	
	public static String getRandomKillMessage(DamageCause cause)
	{
		if (cause == DamageCause.FALL)
		{
			ArrayList<String> messages = new ArrayList<String>();
			
			messages.add("decided to fall off a cliff.");
			messages.add("thought they could walk on air.");
			messages.add("over estimated their health.");
			messages.add("thought they would land in water.");
			messages.add("has fallen and can't get up.");
			messages.add("weeeeeeeeeewwwwww.... splat.");
			messages.add("tripped.");
			messages.add("dun diddley died.");
			
			Random rand = new Random();	
			return messages.get(rand.nextInt(messages.size()-1));
		}
		else if (cause == DamageCause.SUFFOCATION)
		{
			ArrayList<String> messages = new ArrayList<String>();
			
			messages.add("found a hole in the wall... then it closed.");
			messages.add("got a little bit too curious.");
			messages.add("tried to exploit the map.");
			messages.add("found a wall, and went inside of it.");
			
			Random rand = new Random();	
			return messages.get(rand.nextInt(messages.size()-1));
		}
		return "dun diddley died.";
	}
}
