...
public class LobbyWall {
    ...
    private void updateWall(String map2) {
    ...
    public void setAllLines(final String map, final Integer duration, final String[] lines, final Boolean s1, final Boolean s2) {
    	final String admap = plugin.getADmap();
        try {
            if (!cantUpdate.contains(admap)) {
                cantUpdate.add(admap);
            }
            CachedLobbyWallInfo info = getCacheInfo(admap);
            if (info.isDirty()) {
                info.recache();
            }
            ...
            if (s1) {
                ...
                LobbyWallUtilities.getUtilities().setSignLines(signs.get(0), Localizers.getDefaultLoc().LOBBYWALL_JOIN_1.getString(), Localizers.getDefaultLoc().LOBBYWALL_JOIN_2.getString(), Localizers.getDefaultLoc().LOBBYWALL_JOIN_3.getString(), ChatColor.BOLD + "" + admap);
            }
            if (s2) {
    ...
    public String getFriendlyCaptureStatus(String map, Integer id) {
        Map m = plugin.getMap(plugin.getADmap());
